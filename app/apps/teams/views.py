...
@login_required
def pm_list(request):
    """ПМы: кто где ведёт проект — отдельно от общего списка стажёров."""
    pms = (
        TeamMember.objects.filter(role=TeamRole.PROJECT_MANAGER)
        .select_related('intern', 'project')
        .order_by('intern__full_name')
    )
    by_person = {}
    for member in pms:
        ...
        entry['projects'].append(member)
    rows = sorted(by_person.values(), key=lambda e: -len(e['projects']))

    return render(request, 'teams/pm_list.html', {
        'rows': rows,
        'total': len(rows),
        'projects': Project.objects.active().order_by('name'),
        'people': [
...
@login_required
def pm_add(request):
    """Назначить человека ПМом проекта."""
    if request.method != 'POST':
    ...
    data, created_person = _with_new_person(request, role=TeamRole.PROJECT_MANAGER)
    intern = Intern.objects.filter(pk=data.get('intern')).first()
    if intern is None:
        messages.error(request, 'Выберите человека.')
        return redirect('teams:pm_list')
    ...
    if not made and member.role != TeamRole.PROJECT_MANAGER:
    ...
    if created_person:
        messages.success(request, f'{created_person} заведён(а) в базе.')
    messages.success(request, (
        f'{intern.full_name} — ПМ проекта «{project.name}».'
        if made else f'{intern.full_name} уже ПМ в «{project.name}».'
    ))
    return redirect(_safe_next(request) or 'teams:pm_list')


@login_required
def pm_remove(request, pk):
    """Снять роль ПМа: человек остаётся в базе, участие в проекте — нет."""
    member = get_object_or_404(
        TeamMember.objects.select_related('intern', 'project'), pk=pk,
    )
    if request.method == 'POST':
